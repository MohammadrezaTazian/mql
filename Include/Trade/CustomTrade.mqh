...
//+------------------------------------------------------------------+
class CustomCTrade : public CObject
{
protected:
...
public:
   ...
   bool              PositionOpen(const string symbol,const ENUM_ORDER_TYPE order_type,const double volume,
                                  const double price,const double sl,const double tp,const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK);
   ...
   bool              OrderOpen(const string symbol,const ENUM_ORDER_TYPE order_type,const double volume,
                               ...
                               const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK,const int deviation = 0);
   ...
   //--- additions methods
   bool              Buy(const double volume,const string symbol = NULL,double price = 0.0,const double sl = 0.0,const double tp = 0.0,const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK);
   bool              Sell(const double volume,const string symbol = NULL,double price = 0.0,const double sl = 0.0,const double tp = 0.0,const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK);
   bool              BuyLimit(const double volume,const double price,const string symbol = NULL,const double sl = 0.0,const double tp = 0.0,
                              const ENUM_ORDER_TYPE_TIME type_time = ORDER_TIME_GTC,const datetime expiration = 0,const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK);
   ...
   bool              SellStop(const double volume,const double price,const string symbol = NULL,const double sl = 0.0,const double tp = 0.0,
                              const ENUM_ORDER_TYPE_TIME type_time = ORDER_TIME_GTC,const datetime expiration = 0,const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK,const int deviation = 0);
   //--- method check
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CustomCTrade::CustomCTrade(void) : m_async_mode(false),
   m_magic(0),
   m_deviation(0),
...
//| Destructor                                                       |
//+------------------------------------------------------------------+
CustomCTrade::~CustomCTrade(void)
{
}
//+------------------------------------------------------------------+
//| Get the request structure                                        |
//+------------------------------------------------------------------+
void CustomCTrade::Request(MqlTradeRequest &request) const
{
...
//+------------------------------------------------------------------+
string CustomCTrade::RequestActionDescription(void) const
{
   string str;
//---
...
//+------------------------------------------------------------------+
string CustomCTrade::RequestTypeDescription(void) const
{
   string str;
//---
...
//+------------------------------------------------------------------+
string CustomCTrade::RequestTypeFillingDescription(void) const
{
   string str;
//---
...
//+------------------------------------------------------------------+
string CustomCTrade::RequestTypeTimeDescription(void) const
{
   string str;
//---
...
//+------------------------------------------------------------------+
void CustomCTrade::Result(MqlTradeResult &result) const
{
...
//+------------------------------------------------------------------+
string CustomCTrade::ResultRetcodeDescription(void) const
{
   string str;
//---
...
//+------------------------------------------------------------------+
void CustomCTrade::CheckResult(MqlTradeCheckResult &check_result) const
{
//--- copy structure
...
//+------------------------------------------------------------------+
string CustomCTrade::CheckResultRetcodeDescription(void) const
{
   string         str;
...
//| Open position                                                    |
//+------------------------------------------------------------------+
bool CustomCTrade::PositionOpen(const string symbol,const ENUM_ORDER_TYPE order_type,const double volume,
                                const double price,const double sl,const double tp,const string comment,const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK)
{
//--- check stopped
...
//--- setting request
   ...
   m_request.type_filling = type_filling;
//--- check order type
...
//+------------------------------------------------------------------+
bool CustomCTrade::PositionModify(const string symbol,const double sl,const double tp)
{
//--- check stopped
   if(IsStopped(__FUNCTION__))
      return(false);
...
//| Modify specified opened position                                 |
//+------------------------------------------------------------------+
bool CustomCTrade::PositionModify(const ulong ticket,const double sl,const double tp)
{
//--- check stopped
   if(IsStopped(__FUNCTION__))
      return(false);
...
//+------------------------------------------------------------------+
bool CustomCTrade::PositionClose(const string symbol,const ulong deviation)
{
...
//| Close specified opened position                                  |
//+------------------------------------------------------------------+
bool CustomCTrade::PositionClose(const ulong ticket,const ulong deviation)
{
//--- check stopped
   if(IsStopped(__FUNCTION__))
      return(false);
...
//+------------------------------------------------------------------+
bool CustomCTrade::PositionCloseBy(const ulong ticket,const ulong ticket_by)
{
//--- check stopped
   if(IsStopped(__FUNCTION__))
      return(false);
...
//+------------------------------------------------------------------+
bool CustomCTrade::PositionClosePartial(const string symbol,const double volume,const ulong deviation)
{
   uint retcode = TRADE_RETCODE_REJECT;
//--- check stopped
   if(IsStopped(__FUNCTION__))
...
//| Partial close specified opened position (for hedging mode only)  |
//+------------------------------------------------------------------+
bool CustomCTrade::PositionClosePartial(const ulong ticket,const double volume,const ulong deviation)
{
//--- check stopped
   if(IsStopped(__FUNCTION__))
      return(false);
...
//+------------------------------------------------------------------+
bool CustomCTrade::OrderOpen(const string symbol,const ENUM_ORDER_TYPE order_type,const double volume,const double limit_price,
                             const double price,const double sl,const double tp,
                             ENUM_ORDER_TYPE_TIME type_time,const datetime expiration,const string comment,const ENUM_ORDER_TYPE_FILLING type_filling,const int deviation)
{
//--- check stopped
   if(IsStopped(__FUNCTION__))
      return(false);
...
//+------------------------------------------------------------------+
bool CustomCTrade::OrderModify(const ulong ticket,const double price,const double sl,const double tp,
...
//+------------------------------------------------------------------+
bool CustomCTrade::OrderDelete(const ulong ticket)
{
//--- check stopped
   if(IsStopped(__FUNCTION__))
      return(false);
...
//+------------------------------------------------------------------+
void CustomCTrade::PrintRequest(void) const
{
...
//+------------------------------------------------------------------+
void CustomCTrade::PrintResult(void) const
{
   if(m_log_level < LOG_LEVEL_ALL)
      return;
//---
...
//+------------------------------------------------------------------+
void CustomCTrade::ClearStructures(void)
{
...
//+------------------------------------------------------------------+
bool CustomCTrade::IsStopped(const string function)
{
   if(!::IsStopped())
      return(false);
...
//| Buy operation                                                    |
//+------------------------------------------------------------------+
bool CustomCTrade::Buy(const double volume,const string symbol = NULL,double price = 0.0,const double sl = 0.0,const double tp = 0.0,const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK)
{
//--- check volume
   if(volume <= 0.0)
   {
...
//---
   return(PositionOpen(symbol_name,ORDER_TYPE_BUY,volume,price,sl,tp,comment,type_filling));
}
//+------------------------------------------------------------------+
//| Sell operation                                                   |
//+------------------------------------------------------------------+
bool CustomCTrade::Sell(const double volume,const string symbol = NULL,double price = 0.0,const double sl = 0.0,const double tp = 0.0,const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK)
{
//--- check volume
   if(volume <= 0.0)
   {
...
//---
   return(PositionOpen(symbol_name,ORDER_TYPE_SELL,volume,price,sl,tp,comment,type_filling));
}
//+------------------------------------------------------------------+
//| Send BUY_LIMIT order                                             |
//+------------------------------------------------------------------+
bool CustomCTrade::BuyLimit(const double volume,const double price,const string symbol = NULL,const double sl = 0.0,const double tp = 0.0,
                            const ENUM_ORDER_TYPE_TIME type_time = ORDER_TIME_GTC,const datetime expiration = 0,const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK)
{
   string symbol_name;
//--- check volume
...
//| Send BUY_STOP order                                              |
//+------------------------------------------------------------------+
bool CustomCTrade::BuyStop(const double volume,const double price,const string symbol = NULL,const double sl = 0.0,const double tp = 0.0,
                           const ENUM_ORDER_TYPE_TIME type_time = ORDER_TIME_GTC,const datetime expiration = 0,const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK)
{
   string symbol_name;
//--- check volume
...
//--- check symbol
   symbol_name = (symbol == NULL) ? Symbol() : symbol;
//--- send "BUY_STOP" order
   return(OrderOpen(symbol_name,ORDER_TYPE_BUY_STOP,volume,0.0,price,sl,tp,type_time,expiration,comment,type_filling));
}
//+------------------------------------------------------------------+
//| Send SELL_LIMIT order                                            |
//+------------------------------------------------------------------+
bool CustomCTrade::SellLimit(const double volume,const double price,const string symbol = NULL,const double sl = 0.0,const double tp = 0.0,
                             const ENUM_ORDER_TYPE_TIME type_time = ORDER_TIME_GTC,const datetime expiration = 0,const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK)
{
   string symbol_name;
//--- check volume
...
//--- check symbol
   symbol_name = (symbol == NULL) ? Symbol() : symbol;
//--- send "SELL_LIMIT" order
   return(OrderOpen(symbol_name,ORDER_TYPE_SELL_LIMIT,volume,0.0,price,sl,tp,type_time,expiration,comment,type_filling));
}
//+------------------------------------------------------------------+
//| Send SELL_STOP order                                             |
//+------------------------------------------------------------------+
bool CustomCTrade::SellStop(const double volume,const double price,const string symbol = NULL,const double sl = 0.0,const double tp = 0.0,
                            const ENUM_ORDER_TYPE_TIME type_time = ORDER_TIME_GTC,const datetime expiration = 0,const string comment = "",const ENUM_ORDER_TYPE_FILLING type_filling = ORDER_FILLING_FOK,const int deviation = 0)
{
   string symbol_name;
//--- check volume
...
//--- check symbol
   symbol_name = (symbol == NULL) ? Symbol() : symbol;
//--- send "SELL_STOP" order
   return(OrderOpen(symbol_name,ORDER_TYPE_SELL_STOP,volume,0.0,price,sl,tp,type_time,expiration,comment,type_filling,deviation));
}
//+------------------------------------------------------------------+
//| Converts the position type to text                               |
//+------------------------------------------------------------------+
string CustomCTrade::FormatPositionType(string &str,const uint type) const
{
//--- see the type
   switch(type)
   {
...
//+------------------------------------------------------------------+
string CustomCTrade::FormatOrderType(string &str,const uint type) const
{
//--- see the type
   switch(type)
   {
...
//+------------------------------------------------------------------+
string CustomCTrade::FormatOrderTypeFilling(string &str,const uint type) const
{
//--- see the type
   switch(type)
   {
...
//+------------------------------------------------------------------+
string CustomCTrade::FormatOrderTypeTime(string &str,const uint type) const
{
//--- see the type
   switch(type)
   {
...
//+------------------------------------------------------------------+
string CustomCTrade::FormatOrderPrice(string &str,const double price_order,const double price_trigger,const uint digits) const
{
...
//+------------------------------------------------------------------+
string CustomCTrade::FormatRequest(string &str,const MqlTradeRequest &request) const
{
...
//+------------------------------------------------------------------+
string CustomCTrade::FormatRequestResult(string &str,const MqlTradeRequest &request,const MqlTradeResult &result) const
{
//--- set up
   string symbol_name = (request.symbol == NULL) ? _Symbol : request.symbol;
   int    digits = _Digits;
...
//|                                                                  |
//+------------------------------------------------------------------+
double CustomCTrade::CheckVolume(const string symbol,double volume,double price,ENUM_ORDER_TYPE order_type)
{
//--- check
   if(order_type != ORDER_TYPE_BUY && order_type != ORDER_TYPE_SELL)
      return(0.0);
...
//+------------------------------------------------------------------+
bool CustomCTrade::OrderCheck(const MqlTradeRequest &request,MqlTradeCheckResult &check_result)
{
//--- action and return the result
...
//+------------------------------------------------------------------+
bool CustomCTrade::SetTypeFillingBySymbol(const string symbol)
{
...
//+------------------------------------------------------------------+
bool CustomCTrade::FillingCheck(const string symbol)
{
...
//+------------------------------------------------------------------+
bool CustomCTrade::ExpirationCheck(const string symbol)
{
//--- check symbol
   string symbol_name = (symbol == NULL) ? _Symbol : symbol;
//--- get flags
...
//| Checks order                                                     |
//+------------------------------------------------------------------+
bool CustomCTrade::OrderTypeCheck(const string symbol)
{
   bool res = false;
//--- check symbol
   string symbol_name = (symbol == NULL) ? _Symbol : symbol;
...
//| Send order                                                       |
//+------------------------------------------------------------------+
bool CustomCTrade::OrderSend(const MqlTradeRequest &request,MqlTradeResult &result)
{
   bool   res;
   string action = "";
   string fmt   = "";
...
//| Position select depending on netting or hedging                  |
//+------------------------------------------------------------------+
bool CustomCTrade::SelectPosition(const string symbol)
{
   bool res = false;
//---
   if(IsHedging())
